...
class ParticlesReceiver():
    ...
    def recvMCLMessage(self):
        ...
        robot_position = np.array([msg.robot_position.x, 
                                   msg.robot_position.y, 
                                   msg.robot_position.theta])
        mcl_position = np.array([msg.mcl_position.x, 
                                 msg.mcl_position.y, 
                                 msg.mcl_position.theta])
        odometry_position = np.array([msg.odometry_position.x, 
                                      msg.odometry_position.y, 
                                      msg.odometry_position.theta])
        time_steps = msg.time_steps

        particles = []
        for particle in msg.particles:
            p = np.array([particle.w, 
                          particle.x, 
                          particle.y, 
                          particle.theta])
            particles.append(p)
